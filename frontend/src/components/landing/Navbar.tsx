    ...
    <>
      <motion.header
        ...
        className={cn(
          ...
          scrolled
            ? 'bg-[#0A0A0B]/80 backdrop-blur-xl border-b border-white/6 py-3'
            : 'bg-transparent py-5'
        )}
      >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed top-[72px] left-4 right-4 z-[99] bg-[#111113]/95 backdrop-blur-2xl rounded-2xl border border-white/6 shadow-2xl p-3 flex flex-col gap-0.5 md:hidden"
          >
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="px-4 py-3 text-[15px] font-medium text-white/60 hover:text-white rounded-xl hover:bg-white/4 transition-all"
                onClick={() => setMobileMenuOpen(false)}
              >
                {item.label}
              </Link>
            ))}
            <div className="border-t border-white/6 mt-2 pt-3 flex flex-col gap-2 px-1">
              <Link
                href="/sign-in"
                className="flex items-center justify-center h-11 rounded-xl text-white/60 font-medium text-[14px] hover:text-white hover:bg-white/4 transition-colors"
                onClick={() => setMobileMenuOpen(false)}
              >
                Sign in
              </Link>
