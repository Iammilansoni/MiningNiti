...
MOCK_SAFETY_JSON = json.dumps({
    ...
    "confidence": 0.88,
    "hazards": [
        {"type": "atmospheric", "severity": "medium", "description": "Potential methane buildup in Section C", "regulation": "30 CFR 75.323"}
    ],
    "recommendations": ["Install additional methane detectors in Section C"],
    "compliance_details": {"msha_compliant": True, "osha_compliant": True, "dgms_compliant": True, "missing_elements": []},
...
MOCK_SUMMARY_JSON = json.dumps({
    ...
    "action_items": ["Schedule quarterly emergency drill", "Replace methane detectors in Section C"],
...
class TestClassifierAgent:
    ...
    async def test_analyze_returns_expected_keys(self, sample_mining_text):
        ...
        agent.model = make_mock_model(MOCK_CLASSIFICATION_JSON)
    ...
    async def test_analyze_unknown_category_maps_to_other(self, sample_mining_text):
        ...
        from app.agents.classifier import ClassifierAgent
        agent = ClassifierAgent()
        agent.model = make_mock_model(json.dumps({
...
class TestSafetyAnalyzerAgent:
    ...
    async def test_analyze_returns_expected_keys(self, sample_mining_text):
        ...
        agent.model = make_mock_model(MOCK_SAFETY_JSON)

        result = await agent.analyze(sample_mining_text, {"category": "safety_protocol"})

        for key in ("score", "status", "hazards", "recommendations", "compliance_details"):
...
class TestEntityExtractorAgent:
    ...
    async def test_analyze_returns_all_entity_types(self, sample_mining_text):
        ...
        agent.model = make_mock_model(MOCK_ENTITIES_JSON)

        result = await agent.analyze(sample_mining_text)

        for key in ("equipment", "chemicals", "locations", "personnel", "dates", "regulations"):
            assert key in result
    ...
    async def test_entity_count_is_correct(self, sample_mining_text):
        ...
        expected = sum(
            len(result[k])
            for k in ("equipment", "chemicals", "locations", "personnel", "dates", "regulations")
        )
...
class TestAgentOrchestrator:
    ...
    async def test_orchestrator_returns_all_sections(self, sample_mining_text):
        ...
        orchestrator.classifier.model = make_mock_model(MOCK_CLASSIFICATION_JSON)
        orchestrator.safety_analyzer.model = make_mock_model(MOCK_SAFETY_JSON)
        orchestrator.entity_extractor.model = make_mock_model(MOCK_ENTITIES_JSON)
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_orchestrator_handles_agent_failure_gracefully(self, sample_mining_text):
